...
def positional_encoding(inputs,
    ...
    对原文单词的一个位置信息映射，每个位置对应一种position向量，不同batch的相同位置得到的位置编码向量是相同的
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        if zero_pad:  # why
            lookup_table = tf.concat((tf.zeros(shape=[1, num_units]),
                                      lookup_table[1:, :]), 0)
        outputs = tf.nn.embedding_lookup(lookup_table, position_ind)  # [batch_size, seq_length, num_units]

        if scale:
            outputs = outputs * num_units ** 0.5

        return outputs  # [batch_size, seq_length, num_units]
